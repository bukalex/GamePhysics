...
public class PhysicsSystem : MonoBehaviour
{
            ...
            for (int shapeB = shapeA + 1; shapeB < physicsShapes.Count; shapeB++)
            {
                ...
                if (physicsShapes[shapeA].IsOverlapingWithShape(physicsShapes[shapeB], out hitResult) || physicsShapes[shapeB].IsOverlapingWithShape(physicsShapes[shapeA], out hitResult))
                {
                    ...
                    else
                    {
                        ...
                        AddMinimumTranslation(physicsShapes[shapeB], physicsShapes[shapeA], hitResult);
    ...
    private static void ApplyCollisionResponse(PhysicsShape targetShape, PhysicsShape hitShape, ref HitResult hitResult)
    {
        ...
        if (Vector3.Dot(targetShape.Body.Velocity, Vector3.Project(hitResult.impactPoint - targetShape.Position, hitResult.impactNormal)) <= 0) return;
    ...
    private static void AddMinimumTranslation(PhysicsShape shapeA, PhysicsShape shapeB, HitResult hitResult)
    {
